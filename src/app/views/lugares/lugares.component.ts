...
export class LugaresComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    promisePosicoes.then(async (value: any) => {
      this.map = L.map('map', {
      ...
      //marcadores
      L.marker([value.latitude, value.longitude], { icon: this.violetIcon })
      .addTo(this.map)
    ...
    if(this.pets && this.pets.pets){
      for(const pet of this.pets.pets){
        if(pet.latitude && pet.longitude){
          L.marker([pet.latitude, pet.longitude], { icon: this.violetIcon })
      .addTo(this.map)
      .bindPopup(pet.nome);
        }
      }
